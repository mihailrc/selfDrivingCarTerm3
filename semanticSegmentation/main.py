...
# Check for a GPU - yeah I did not have one with sufficient memory so training it was kinda slow.
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    l7_conv_1x1 = tf.layers.conv2d(vgg_layer7_out, num_classes,
                                kernel_size=kernel, strides=stride, padding='same',
                                kernel_initializer = kernel_init,
                                kernel_regularizer = l2_regularizer)
    # Decoder
    l7_trans = tf.layers.conv2d_transpose(l7_conv_1x1, num_classes, 4, 2, padding='same',
                                kernel_initializer = kernel_init,
                                kernel_regularizer = l2_regularizer)
    ...
    l4_conv_1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes,
                                ...
                                kernel_initializer = kernel_init,
                                kernel_regularizer = l2_regularizer)
    skip_4 = tf.add(l7_trans, l4_conv_1x1)

    l4_trans = tf.layers.conv2d_transpose(skip_4, num_classes, 4, 2, padding='same',
                                kernel_initializer = kernel_init,
                                kernel_regularizer = l2_regularizer)
    ...
    skip_3 = tf.add(l4_trans, l3_conv_1x1)

    output = tf.layers.conv2d_transpose(skip_3, num_classes, 16, 8, padding='same',
                                kernel_initializer = kernel_init,
                                kernel_regularizer = l2_regularizer)
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    expected_labels = tf.reshape(correct_label, (-1, num_classes))

    # Softmax with cross entropy loss
    softmax_logits = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=expected_labels)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for batch, (image, label) in enumerate(get_batches_fn(batch_size)):
            ...
            if batch%16 == 0:
                print("Epoch: {:<5} Batch: {:<5} Loss: {:<10.4f} "
                      "Time: {:<.1f} seconds".format(epoch+1,batch+1,loss,time.time()-batch_start))
